	...
	public class XObjectConverter : JsonConverter
	{
		...
		public Func<string, string, object> GetValue
		{
			get
			{
				if (_getValue == null)
				{
					return new Func<string, string, object>((string v, string p) => v);
				}
				else
				{
					return _getValue;
		...
		private void CreateElement(JsonReader reader, IXmlDocument document, IXmlNode currentNode, string elementName, XmlNamespaceManager manager, string elementPrefix, Dictionary<string, string> attributeNameValues)
		{
			...
			foreach (var attributeNameValue in attributeNameValues)
			{
				...
				string prefix = Utils.GetPrefix(attributeNameValue.Key);
							...
							if (!string.IsNullOrEmpty(str1))
							{
								switch (str1[0])
								{
									...
									case '@':
										...
										string prefix1;
										if (IsNamespaceAttribute(str4, out prefix1))
										{
